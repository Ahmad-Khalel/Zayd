...
const SITE = {
  name: "ثانوية زيد بن حارثة للتعليم الشرعي",
  phone: "0939 623 830",
  whatsapp: "963939623830",
  email: "info@zayd-school.example",
  address: "سورية - يمكن تعديل العنوان من لوحة الإدارة",
  donationAccount: "0000 0000 0000",
  socials: {
...
function layout(content, active = "") {
  const links = [
    ["#", "الرئيسية", "home"], ["#about", "عن المدرسة", "about"], ["#admissions", "الانتساب", "admissions"],
    ["#curriculum", "المناهج", "curriculum"], ["#news", "الأخبار", "news"], ["#contact", "تواصل معنا", "contact"],
  ];
  return `
    <a class="skip-link" href="#main">تجاوز إلى المحتوى</a>
    <div class="topbar"><div class="container topbar-inner">
      <span>العلم النافع، والخلق الرفيع، وبناء الإنسان</span>
      <div class="topbar-links"><a href="tel:${SITE.phone.replaceAll(" ", "")}">الهاتف: ${SITE.phone}</a><span>•</span><a href="mailto:${SITE.email}">${SITE.email}</a></div>
    </div></div>
    <header class="navbar"><div class="container nav-inner">
      <a class="brand" href="#">
        ...
        <span><strong>ثانوية زيد بن حارثة</strong><span>للتعليم الشرعي</span></span>
      </a>
      ...
      <div class="nav-actions"><button class="icon-btn" id="theme-toggle" title="تبديل الوضع" aria-label="تبديل الوضع">◐</button><button class="menu-btn" id="menu-toggle" aria-label="فتح القائمة">☰</button></div>
    </div></header>
    <main id="main">${content}</main>
    <a class="whatsapp" href="https://wa.me/${SITE.whatsapp}" target="_blank" rel="noreferrer" aria-label="التواصل عبر واتساب">◔</a>
    ${footer()}
  `;
}
...
function footer() {
  return `<footer class="footer"><div class="container">
    <div class="footer-grid">
      <div><a class="brand" href="#"><img src="assets/logo-profile.png" alt="شعار المدرسة" /><span><strong>ثانوية زيد بن حارثة</strong><span>للتعليم الشرعي</span></span></a><p>صرح تربوي يجمع بين أصالة العلم الشرعي وجودة التعليم، ويصنع بيئة آمنة لنمو الطالب علماً وخلقاً.</p></div>
      <div><h3>روابط سريعة</h3><div class="footer-links"><a href="#about">عن المدرسة</a><a href="#admissions">التسجيل والانتساب</a><a href="#curriculum">المناهج الدراسية</a><a href="#parents">بوابة الأهل</a></div></div>
      <div><h3>تواصل معنا</h3><div class="footer-links"><a href="tel:${SITE.phone.replaceAll(" ", "")}">${SITE.phone}</a><a href="mailto:${SITE.email}">${SITE.email}</a><span>${SITE.address}</span><div class="socials">${Object.entries(SITE.socials).map(([name, url]) => `<a href="${url}" target="_blank" rel="noreferrer">${name}</a>`).join("")}</div></div></div>
    </div>
    <div class="footer-bottom"><span>© ${new Date().getFullYear()} ثانوية زيد بن حارثة. جميع الحقوق محفوظة.</span><span>موقع تعليمي مستقل يعمل دون خادم</span></div>
  </div></footer>`;
}
...
function homePage() {
  ...
  return layout(`
    <section class="hero"><div class="container hero-grid">
      <div><span class="eyebrow">بوابة العلم والتربية</span><h1>نبني جيلاً راسخ العلم، <span>رفيع الخلق</span></h1><p>في ثانوية زيد بن حارثة نرافق أبناءنا في رحلة تجمع القرآن والعلم، وتمنحهم أساساً متيناً لمستقبل واثق ومشرق.</p><div class="hero-actions"><a class="btn btn-primary" href="#admissions">ابدأ رحلة التسجيل ←</a><a class="btn btn-secondary" href="#about">تعرّف إلى المدرسة</a></div></div>
      <div class="hero-art"><img class="hero-logo" src="assets/logo.png" alt="شعار ثانوية زيد بن حارثة" onerror="this.src='assets/logo-profile.png'" /><div class="hero-badge one">تعليم شرعي وأكاديمي</div><div class="hero-badge two">بيئة آمنة ومحفزة</div></div>
    </div></section>
    <section class="stats"><div class="container stats-grid"><div class="stat"><strong>10+</strong><span>سنوات من العطاء</span></div><div class="stat"><strong>12</strong><span>مساراً تعليمياً</span></div><div class="stat"><strong>100%</strong><span>اهتمام ومتابعة</span></div><div class="stat"><strong>24/7</strong><span>تواصل مع الأهل</span></div></div></section>
...
function newsCard(item) {
  return `<article class="card news-card"><div class="news-thumb">✦</div><div class="news-content"><span class="news-date">${formatDate(item.date)} · ${escapeHtml(item.category)}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.text)}</p><button class="btn btn-ghost btn-sm" data-news="${item.id}">قراءة المزيد</button></div></article>`;
}
...
function aboutPage() {
  return layout(`<section class="page-hero"><div class="container"><span class="breadcrumbs">الرئيسية / عن المدرسة</span><h1>عن ثانوية زيد بن حارثة</h1><p class="lead">تربية تجمع بين أصالة المنهج الشرعي ومتطلبات التعليم الحديث.</p></div></section>
    ...
    <section class="section"><div class="container"><div class="section-head center"><span class="section-kicker">قيمنا</span><h2>قيم نعيشها كل يوم</h2></div><div class="values-grid">${[["◈","الأمانة","نؤدي رسالتنا بإخلاص ومسؤولية."],["✦","الإتقان","نبحث عن أفضل طريقة ليتعلم الطالب."],["♡","الرحمة","نربي بالحوار والاحترام والاحتواء."],["⌂","الانتماء","نصنع مجتمعاً مدرسياً متعاوناً."]].map(([i,t,p]) => `<article class="card value-card"><div class="feature-icon purple">${i}</div><h3>${t}</h3><p>${p}</p></article>`).join("")}</div></div></section>
    <section class="section"><div class="container"><div class="section-head"><span class="section-kicker">رؤيتنا ورسالتنا</span><h2>منهج واضح، وأثر ممتد</h2></div><div class="features-grid"><article class="card"><div class="feature-icon">◎</div><h3>الرؤية</h3><p>الريادة في بناء جيل مسلم واعٍ، متعلم، وفاعل في مجتمعه.</p></article><article class="card"><div class="feature-icon purple">◇</div><h3>الرسالة</h3><p>تقديم تعليم شرعي وأكاديمي متكامل ضمن بيئة آمنة ومحفزة.</p></article><article class="card"><div class="feature-icon">↗</div><h3>الهدف</h3><p>تمكين الطالب من أدوات العلم والشخصية ليواصل طريقه بثقة.</p></article></div></div></section>`, "about");
}
...
function adminAdmissions(items) {
  return `<div class="admin-head"><div><span class="section-kicker">الانتساب</span><h2>طلبات التسجيل</h2></div><button class="btn btn-ghost btn-sm" data-export="admissions">تصدير CSV</button></div><div class="card admin-table">${items.length ? `<table><thead><tr><th>الطالب</th><th>الصف</th><th>الهاتف</th><th>التاريخ</th><th>الإجراء</th></tr></thead><tbody>${items.map((a) => `<tr><td>${escapeHtml(a.firstName)} ${escapeHtml(a.fatherName)} ${escapeHtml(a.familyName)}</td><td>${escapeHtml(a.grade)}</td><td>${escapeHtml(a.phone)}</td><td>${formatDate(a.submittedAt)}</td><td><button class="btn btn-danger btn-sm" data-delete-admission="${a.id}">حذف</button></td></tr>`).join("")}</tbody></table>` : `<div class="empty">لا توجد طلبات تسجيل حتى الآن.</div>`}</div>`;
}
function adminMessages(items) {
  return `<div class="admin-head"><div><span class="section-kicker">صندوق الوارد</span><h2>رسائل التواصل</h2></div></div><div class="card">${items.length ? items.map((m) => `<div class="admin-item"><div><strong>${escapeHtml(m.name)} · ${escapeHtml(m.subject)}</strong><p class="muted">${escapeHtml(m.message)}</p></div><button class="btn btn-danger btn-sm" data-delete-message="${m.id}">حذف</button></div>`).join("") : `<div class="empty">لا توجد رسائل بعد.</div>`}</div>`;
}
function adminStudents(items) {
  return `<div class="admin-head"><div><span class="section-kicker">السجل الدراسي</span><h2>بيانات الطلاب</h2></div><button class="btn btn-primary btn-sm" id="add-student">إضافة طالب</button></div><div class="card admin-table">${items.length ? `<table><thead><tr><th>الاسم</th><th>الصف</th><th>الهاتف</th><th>الحضور</th><th>الإجراء</th></tr></thead><tbody>${items.map((s) => `<tr><td>${escapeHtml(s.name)}</td><td>${escapeHtml(s.grade)}</td><td>${escapeHtml(s.phone)}</td><td>${s.attendance}%</td><td><button class="btn btn-danger btn-sm" data-delete-student="${s.id}">حذف</button></td></tr>`).join("")}</tbody></table>` : `<div class="empty">لا يوجد طلاب.</div>`}</div>`;
}
function adminNews(items) {
  return `<div class="admin-head"><div><span class="section-kicker">المحتوى</span><h2>الأخبار والإعلانات</h2></div><button class="btn btn-primary btn-sm" id="add-news">إضافة خبر</button></div><div class="card">${items.map((n) => `<div class="admin-item"><div><strong>${escapeHtml(n.title)}</strong><small class="muted"> · ${formatDate(n.date)}</small></div><button class="btn btn-danger btn-sm" data-delete-news="${n.id}">حذف</button></div>`).join("")}</div>`;
}
...
function bindCommon(route) {
  ...
  $("#theme-toggle")?.addEventListener("click", () => { document.body.classList.toggle("dark"); localStorage.setItem("zayd_dark", document.body.classList.contains("dark") ? "1" : "0"); });
...
function openModal(content) {
  const modal = document.createElement("div"); modal.className = "modal-backdrop"; modal.innerHTML = `<div class="modal" role="dialog" aria-modal="true"><div class="modal-head"><span></span><button class="close-btn" aria-label="إغلاق">×</button></div>${content}</div>`;
